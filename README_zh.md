...
- **免费就是免费**：目录里发布了下载地址的，工具直接给出来，不注册、不设门。
  （偶尔会有免费条目还没发布压缩包，那时它会如实说明。）
- **付费只报价，不劝**：没有紧迫感话术，没有推荐语。付费项的下载链接在付款后由订单页和邮件发出。

  具体做法：目录里凡是没有明确标成 `free` 的一律按付费对待（fail closed），下载字段直接清空，
  名称、结果说明、安装命令、来源、仓库这几个字段里的网址被剥除，拼好的输出再按**路由白名单**核一遍。
  能留下的只有 `https://agentskill.nz`（可带 `/zh`）下的：站点根、`/products/<handle>`、
  `/collections/<handle>`、`/pages/<handle>`、`/cart` 或 `/cart/<变体>:<数量>`——而且是**整条路由**匹配，
  所以像 `/products/x/download` 这种更深的路径也不算。其它一律让服务拒绝返回这条记录：
  换个域名不行、明文 http 不行、相似域名不行、`/download?id=…` 也不行。

  **这道边界不覆盖的**：没有协议头的裸主机名（`evil.example/x`）。要拦它就得把所有"长得像域名"
  的文本当地址，而 `.md` 是真实存在的顶级域——交付清单里的 `SKILL.md` 会被判成域名，
  于是每一条付费详情都发不出来。这些响应是纯文本，裸主机名只有在下游主动把它变成链接时才成为地址。

  **有一个缺口值得说明**：目录最多缓存五分钟。在这个窗口内刚转成付费的条目，仍会按上一份快照
  来描述——**包括它还免费时的那个直接下载地址**。
- **安装命令是信息，不是授权**：`get_capability`（唯一会返回安装命令的工具）每次都会写明这一点。
  动你的文件系统或花你的钱之前，agent 应该先问你。
